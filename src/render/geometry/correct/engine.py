...
from ..verify.rules import Violation, Severity, VerifyContext
from ..verify.engine import verify, has_critical
...
def correct(ctx: VerifyContext, max_iterations: int = MAX_ITERATIONS) -> CorrectionResult:
    ...
    if not original_violations:
        return CorrectionResult(
            original_violations=[], applied=[],
            remaining_violations=[], grid=ctx.grid, boxes=ctx.boxes
        )
    ...
    for iteration in range(max_iterations):
        ...
        if not fixable:
            if critical:
                return CorrectionResult(
                    ...
                    grid=current_grid, boxes=current_boxes,
                    needs_relayout=True,
                )
            break
        ...
        applied_fixes.append(f"iter_{iteration}: {fix_count} fixes via shift/resize/reroute/collapse")
...
def _make_ctx(grid, boxes, edges, parent_ctx):
    return VerifyContext(
        grid=grid, boxes=boxes, edges=edges,
        groups=parent_ctx.groups, modules=parent_ctx.modules,
