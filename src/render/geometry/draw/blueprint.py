...
def render_blueprint(ctx: VerifyContext) -> str:
    ...
    control, core, process, support = _partition(
        nodes, edges, getattr(ctx, "focus", None)
    )

    ordered = control + [core] + process + support
    digits = max(2, len(str(len(ordered))))
    refs = {
        node: str(index).zfill(digits)
    ...
    for row in range(body_h + 2):
        ...
        output.append(
            blocks[0][row] + to_core + blocks[1][row]
            + to_process + blocks[2][row]
        )
...
def _partition(
    ...
    connected = [
        node for node in nodes if incoming[node] or outgoing[node]
    ]
    if focus in nodes:
        core = focus
    elif connected:
