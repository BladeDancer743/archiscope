...
from ..draw.grid import CharGrid, Rect, str_width, pad_str, SINGLE, DOUBLE, DASHED
...
def render_nested(ctx: VerifyContext, style: str = "grouped") -> str:
    ...
    # Run verify + correct
    vctx = VerifyContext(grid=grid, boxes=boxes, edges=edges,
                         groups=groups, modules=modules, terminal_width=terminal_w)
    result = correct(vctx)
