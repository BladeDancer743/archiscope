...
from .grid import (CharGrid, Rect, str_width, pad_str, truncate_str,
                   split_lines, ARROW_RIGHT)
...
def assign_layers(nodes: list[str], edges: list[dict]) -> dict[str, int]:
    ...
    return {n: max_l - l for n, l in layer.items()}


def order_within_layers(layers: list[list[str]], edges: list[dict],
                        passes: int = 3) -> list[list[str]]:
...
def render_topology(ctx: VerifyContext, style: str = "flow") -> str:
    ...
    for col in columns:
        ...
        for n in col:
            ...
            lines = (split_lines(label, MAX_LABEL_W)
                     if str_width(label) > MAX_LABEL_W else [label])
            lines = [truncate_str(l, MAX_LABEL_W) for l in lines[:2]]
            wmax = max(wmax, max(str_width(l) for l in lines))
    ...
    for e in sorted(edges, key=lambda e: abs(col_index[e["to"]] - col_index[e["from"]])):
        ...
        if c1 == c2:
            ...
            for xx in range(r1.right, lane + 1):
                grid.put(xx, sy, "─"); cells.append((xx, sy))
            lo, hi = sorted((sy, ty))
            for yy in range(lo, hi + 1):
                grid.put(lane, yy, "│"); cells.append((lane, yy))
            grid.put(lane, sy, "┐" if ty > sy else "┘")
            grid.put(lane, ty, "└" if ty > sy else "┌")
            for xx in range(r2.right + 1, lane):
                grid.put(xx, ty, "─"); cells.append((xx, ty))
