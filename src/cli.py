...
def main():
    ...
    render_cmd.add_argument("--zoom", default="auto",
                            choices=["auto", "panorama", "engine", "layer", "module", "function"],
                            help="Override zoom level")
    render_cmd.add_argument("--strategy", default=None,
                            help="Force a render strategy (e.g. swimlane, grouped, tree)")
    # list-strategies
    ...
    val_cmd.add_argument("--path", default=None, help="Path to .archmap.yaml (auto-detect by default)")
    ...
    inst_cmd.add_argument("--agents", nargs="+",
        ...
        try:
            ...
            if actual not in modules:
                ...
                raise UserInputError(
                    f"Module '{args.path}' not found. Available: {available}"
                )
...
def _install_detect():
    from .install import INSTALL_MAP, find_project, detect_agents
    root = find_project()
    if not root:
        print("Not in a project directory")
        return
